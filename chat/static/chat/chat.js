...
const currentUser = JSON.parse(document.getElementById('user-name').textContent);
        ...
        chatSocket.onmessage = function(e) {
            ...
            if(data.type==='chat_message'){
                if(Array.isArray(data.message)){
                    ...
                    which_sender = element.sender
                    const chatLogs = document.querySelector('#chat-logs');
                    const chatLog = document.createElement('li');
                    chatLog.style.listStyleType = 'none';
                    let message = `${which_sender}: ${element.content}`;
                    if (which_sender == currentUser){
                        chatLog.style.textAlign = "right";
                        message = `${element.content} :${which_sender}`;
